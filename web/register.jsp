...
<script language="javascript" src="${pageContext.request.contextPath}/js/AjaxRequest.js"></script>
<script language="javascript" src="${pageContext.request.contextPath}/js/wghFunction.js"></script>
...
function Regopen(divID){
	...
	divID.style.left=(document.body.clientWidth-863)/2+"px";		//设置页面的左边距
...
function checkUser(str){
	if(str==""){			//当用户名为空时
		...
		document.getElementById("tr_user").style.display='table-row';	//显示提示信息
		flag_user=false;
...
function checkPwd(str){
	if(str==""){		//当密码为空时
		...
		document.getElementById("tr_pwd").style.display='table-row';		//显示提示信息
		flag_pwd=false;
		...
		document.getElementById("tr_pwd").style.display='table-row';		//隐藏提示信息显示行
		flag_pwd=true;
	}
}
//验证确认密码是否正确
...
function checkEmail(str){
	if(str==""){//当E-mail地址为空时
		...
		document.getElementById("tr_email").style.display='table-row';		//显示提示信息
		flag_email=false;
...
function checkQuestion(str_q,str_a){
	if(str_q!="" && str_a==""){//当密码提示问题不为空，而提示问题答案为空时
		...
		document.getElementById("tr_answer").style.display='table-row';		//显示提示信息
		flag_answer=false;
	}else if(str_q=="" && str_a !=""){//当密码提示问题为空，而提示问题答案不为空时
		document.getElementById("div_question").innerHTML="请输入密码提示问题！";//设置提示信息
		document.getElementById("tr_question").style.display='table-row';	//显示提示信息
		flag_question=false;
	}else{
...
function form_reset(form){
  ...
  <div id="register" style="width:915px; height:433px; background-color:#546B51; padding:4px; position:absolute; z-index:11;display:none;">
    ...
    <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FEFEFC">
      <tr>
        <td height="408" align="center" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td height="50" style="color:#1B7F5D;font-size: 16px;padding-left:27px;"><b>九宫格记忆网--用户注册</b></td>
          </tr>
        </table>
            ...
            <tr>
              <td height="310" valign="top" bgcolor="#FFFFFF"><table border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFEF9">
                ...
                <tr id="tr_email" style="display:none">
                  <td height="40" colspan="2" align="center"><div id="div_email" style="border:#FF6600 1px solid; color:#FF0000; width:90%; height:29px; padding-top:8px; "></div></td>
                </tr>
                <tr>
                ...
                <tr id="tr_question" style="display:none">
                  <td height="40" colspan="2" align="center"><div id="div_question" style="border:#FF6600 1px solid; color:#FF0000; width:90%; height:29px; padding-top:8px;"></div></td>
                </tr>
                <tr>
                ...
                <tr id="tr_answer" style="display:none">
                  <td height="40" colspan="2" align="center"><div id="div_answer" style="border:#FF6600 1px solid; color:#FF0000; width:90%; height:29px; padding-top:8px;"></div></td>
                </tr>
                <tr>
