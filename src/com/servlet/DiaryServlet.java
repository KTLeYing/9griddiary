...
import com.entity.Commnet;
import com.entity.Diary;
import com.utils.MyPagination;
	...
	private void delDiary(HttpServletRequest request,
		...
		if (rtn > 0) {// 当删除日记成功时
			...
			String path = getServletContext().getRealPath("\\")
					+ "images\\diary\\";
			File file = new File(path + imgName + "scale.jpg");// 获取缩略图
			file.delete(); // 删除指定的文件
...
//		System.out.println(request.getRealPath("/"));
		...
		int num = string.indexOf("9griddiary");
		String url = string.substring(1,num).replace('/', '\\') + "9griddiary\\web\\";
		System.out.println(url);

//		String url = request.getRealPath("/");// 获取请求的实际地址
